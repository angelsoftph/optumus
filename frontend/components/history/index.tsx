...
import UserHistoryBookComponent from "./book";
...
const UserHistoryComponent: React.FC = () => {
  ...
  const fetchBooks = async ({
    ...
    const response = await fetchWithAuth(
      `${API_URL}/api/user-history?limit=${limit}&offset=${offset}`
    );
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4">
        {history.map((book: LoanedBookType, index) => (
          <div key={index} className="w-full">
            <UserHistoryBookComponent bookData={book} />
